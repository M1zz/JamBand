...
기기가 1대뿐이어도 전체 합주를 보여 줄 수 있습니다.

- 로비 오른쪽 위 **데모** 버튼을 누르면 가상 플레이어(🤖 봇)가 내가 고르지 않은 악기를 모두 맡습니다.
- 봇들이 차례로 준비 완료가 되고, 내가 "준비 완료"를 누르면 3초 후 **6파트 전체가 이 기기에서** 함께 재생됩니다.
- 실제 기기가 접속하면 데모 모드는 자동으로 꺼집니다. 다른 기기가 연결돼 있을 때는 켤 수 없습니다.
- Xcode 스킴의 Arguments에 `-DemoMode`를 넣으면 앱이 데모 모드로 바로 시작됩니다.
